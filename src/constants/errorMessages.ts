export const ERROR_REQUIRED = 'This field is required.'
export const ERROR_FIRST_NAME = 'Please enter the last name.'
export const ERROR_LAST_NAME = 'Please enter the last name.'
export const ERROR_COUNTRY = 'Please enter the country.'
export const ERROR_ADDRESS = 'Please enter the address.'
export const ERROR_CITY = 'Please enter the city.'
export const ERROR_STATE = 'Please enter the country/State.'
export const ERROR_PAYMENT_METHOD = 'Please select a payment method.'
export const ERROR_ZIPCODE_DIGITS_ONLY = 'Must be only digits'
export const ERROR_ZIPCODE_LENGTH = 'Must be exactly 5 digits'
export const ERROR_PHONE = 'Please enter the Phone number'
export const ERROR_PHONE_INVALID = 'Phone number is not valid'
export const ERROR_EMAIL = 'Please enter the email'
export const ERROR_EMAIL_INVALID = 'The email is not valid'

/***************AUTH********************/
export const ERROR_AUTH_DISPLAY_NAME = 'Please enter the display name'
export const ERROR_AUTH_USERNAME = 'Please enter the username'
export const ERROR_AUTH_PASSWORD = 'Please enter the password'
export const ERROR_AUTH_CONFIRM_PASSWORD = 'Please enter the Repeat password'